...
#                     TO TEST 
# ----------------------------------------------------- #
# Raw code without loop :

{
  #TRAINING
  timerStart("RF TRAIN")
  svm_res <- ksvm(x=pca_res$x[,1:PCA], y=train$labels, kernel=KERNEL, C=C)
  print(svm_res) 
  time_rfTrainDuration <- timerEnd("")
  
  #TESTING
  timerStart("RF TEST")
  svm_pred <- predict(svm_res, newdata = pca_pred[,1:PCA], type = "response")
  time_rfTestDuration <- timerEnd("")
...
#svm_model <- ksvm(train$labels ~ pca_res$x[,1] + pca_res$x[,2],data = data, kernel=KERNEL, C=C)

#data[, 1:2] = sapply(data[,1:2], scale)
library("plot3D")
scatter3D(x = pca_pred[,1], y = pca_pred[,2], z = pca_pred[,3], 
          colvar = as.numeric(svm_pred))
